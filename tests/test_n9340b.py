...
class TestReadingCSVFiles(unittest.TestCase):
    ...
    def setUp(self):
        test_csv_file = Path(TEST_DIR, 'sample_data', 'N9340B.CSV')
        print(test_csv_file)
        (self.n9340_header, self.n9340_data) = n9340.read_csv_file(
            test_csv_file)

    def test_header_when_reading_n9340_file(self):
        self.assertEqual(self.n9340_header['file'],
                         '/bd0/N9340DATA/A10.CSV')
        self.assertEqual(self.n9340_header['system_parameter'],
                         'Default Unit:dBm/Hz/s')
    ...
    def test_data_when_reading_n9340_file(self):
        ...
        self.assertEqual(self.n9340_data['amplitude_db'][0], -43.73)
        self.assertEqual(self.n9340_data['frequency'][1], 165000)
        self.assertEqual(self.n9340_data['amplitude_db'][1], -52.12)
        self.assertEqual(self.n9340_data['frequency'][-1], 30000000)
        self.assertEqual(self.n9340_data['amplitude_db'][-1], -73.79)
