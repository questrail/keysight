...
class TestReadingCSVFiles(unittest.TestCase):
    ...
    def setUp(self):
        test_csv_file = Path(TEST_DIR, 'sample_data', 'N9038A_3.CSV')
        print(test_csv_file)
        (self.header, self.data) = n9038.read_csv_file(
            test_csv_file)

    def test_header_when_reading_n9038_file(self):
        self.assertEqual(self.header['data_file_type'],
                         'AllTrace')
        self.assertEqual(self.header['instrument_ver'],
                         'A.25.08')
        self.assertEqual(self.header['model_num'],
                         'N9038A')
